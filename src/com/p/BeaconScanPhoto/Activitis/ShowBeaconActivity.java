...
public class ShowBeaconActivity extends Activity {
    ...
    private static class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            final ShowBeaconActivity activity = mActivity.get();
            if (activity != null) {
                switch (msg.what) {
                    ...
                    case KEY_TIME_OUT:
                        new AlertDialog.Builder(activity)
                                .setTitle("提示")
                                .setMessage("当前登录已经超时，是否重新登录").setCancelable(false)
                                .setPositiveButton("确定", new DialogInterface.OnClickListener() {

                                    @Override
                                    public void onClick(DialogInterface dialog, int which) {
                                        // TODO Auto-generated method stub
                                        PublicData.getInstance().setLogin(false);
                                        activity.onButtonClicked(activity.upload_button);
                                    }
